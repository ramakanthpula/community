...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.allotUnit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.allot-units.update", [$allotUnit->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users_id" id="users_id" required>
                    @foreach($users as $id => $users)
                        <option value="{{ $id }}" {{ (old('users_id') ? old('users_id') : $allotUnit->users->id ?? '') == $id ? 'selected' : '' }}>{{ $users }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('units') ? 'is-invalid' : '' }}" name="units_id" id="units_id" required>
                    @foreach($units as $id => $units)
                        <option value="{{ $id }}" {{ (old('units_id') ? old('units_id') : $allotUnit->units->id ?? '') == $id ? 'selected' : '' }}>{{ $units }}</option>
                    @endforeach
                </select>
                @if($errors->has('units'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unitnames') ? 'is-invalid' : '' }}" type="text" name="unitnames" id="unitnames" value="{{ old('unitnames', $allotUnit->unitnames) }}" required>
