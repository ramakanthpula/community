    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.meetings.update", [$meeting->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('from') ? 'is-invalid' : '' }}" type="text" name="from" id="from" value="{{ old('from', $meeting->from) }}" required>
                @if($errors->has('from'))
                    <div class="invalid-feedback">
                        {{ $errors->first('from') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('to') ? 'is-invalid' : '' }}" type="text" name="to" id="to" value="{{ old('to', $meeting->to) }}" required>
                @if($errors->has('to'))
                    <div class="invalid-feedback">
                        {{ $errors->first('to') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('venue') ? 'is-invalid' : '' }}" type="text" name="venue" id="venue" value="{{ old('venue', $meeting->venue) }}" required>
                @if($errors->has('venue'))
                    <div class="invalid-feedback">
                        {{ $errors->first('venue') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $meeting->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Meeting::ATTENDEES_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('attendees') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="attendees_{{ $key }}" name="attendees" value="{{ $key }}" {{ old('attendees', $meeting->attendees) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_user_group') ? 'is-invalid' : '' }}" name="select_user_group_id" id="select_user_group_id">
                    @foreach($select_user_groups as $id => $entry)
                        <option value="{{ $id }}" {{ (old('select_user_group_id') ? old('select_user_group_id') : $meeting->select_user_group->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('specific_users') ? 'is-invalid' : '' }}" name="specific_users_id" id="specific_users_id">
                    @foreach($specific_users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('specific_users_id') ? old('specific_users_id') : $meeting->specific_users->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
