...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.helpDesk.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.help-desks.update", [$helpDesk->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.helpDesk.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $helpDesk->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mobile') ? 'is-invalid' : '' }}" type="text" name="mobile" id="mobile" value="{{ old('mobile', $helpDesk->mobile) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('flat_no') ? 'is-invalid' : '' }}" type="text" name="flat_no" id="flat_no" value="{{ old('flat_no', $helpDesk->flat_no) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('timestamp') ? 'is-invalid' : '' }}" type="text" name="timestamp" id="timestamp" value="{{ old('timestamp', $helpDesk->timestamp) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('enquiry_types') ? 'is-invalid' : '' }}" name="enquiry_types[]" id="enquiry_types" multiple required>
                    @foreach($enquiry_types as $id => $enquiry_type)
                        <option value="{{ $id }}" {{ (in_array($id, old('enquiry_types', [])) || $helpDesk->enquiry_types->contains($id)) ? 'selected' : '' }}>{{ $enquiry_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('details') ? 'is-invalid' : '' }}" name="details" id="details" required>{{ old('details', $helpDesk->details) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $helpDesk->description) }}</textarea>
