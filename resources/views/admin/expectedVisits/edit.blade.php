...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.expectedVisit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.expected-visits.update", [$expectedVisit->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                @foreach(App\Models\ExpectedVisit::VISIT_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('visit_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="visit_type_{{ $key }}" name="visit_type" value="{{ $key }}" {{ old('visit_type', $expectedVisit->visit_type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unit_no') ? 'is-invalid' : '' }}" name="unit_no_id" id="unit_no_id">
                    @foreach($unit_nos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('unit_no_id') ? old('unit_no_id') : $expectedVisit->unit_no->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $expectedVisit->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_of_persons') ? 'is-invalid' : '' }}" type="text" name="no_of_persons" id="no_of_persons" value="{{ old('no_of_persons', $expectedVisit->no_of_persons) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender" required>
                    ...
                    @foreach(App\Models\ExpectedVisit::GENDER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gender', $expectedVisit->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address" id="address">{{ old('address', $expectedVisit->address) }}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ExpectedVisit::CHECK_IN_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('check_in_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="check_in_type_{{ $key }}" name="check_in_type" value="{{ $key }}" {{ old('check_in_type', $expectedVisit->check_in_type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ExpectedVisit::VISITING_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('visiting_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="visiting_type_{{ $key }}" name="visiting_type" value="{{ $key }}" {{ old('visiting_type', $expectedVisit->visiting_type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ExpectedVisit::CHECK_IN_BY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('check_in_by') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="check_in_by_{{ $key }}" name="check_in_by" value="{{ $key }}" {{ old('check_in_by', $expectedVisit->check_in_by) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ExpectedVisit::VEHICLE_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('vehicle_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="vehicle_type_{{ $key }}" name="vehicle_type" value="{{ $key }}" {{ old('vehicle_type', $expectedVisit->vehicle_type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('expected_in_date') ? 'is-invalid' : '' }}" type="text" name="expected_in_date" id="expected_in_date" value="{{ old('expected_in_date', $expectedVisit->expected_in_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('expected_in_time') ? 'is-invalid' : '' }}" type="text" name="expected_in_time" id="expected_in_time" value="{{ old('expected_in_time', $expectedVisit->expected_in_time) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('purposes') ? 'is-invalid' : '' }}" name="purposes[]" id="purposes" multiple>
                    @foreach($purposes as $id => $purpose)
                        <option value="{{ $id }}" {{ (in_array($id, old('purposes', [])) || $expectedVisit->purposes->contains($id)) ? 'selected' : '' }}>{{ $purpose }}</option>
                    @endforeach
                </select>
