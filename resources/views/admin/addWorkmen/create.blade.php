...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.addWorkman.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.add-workmen.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\AddWorkman::CATEGORY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('category') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="category_{{ $key }}" name="category" value="{{ $key }}" {{ old('category', '') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('skilled') ? 'is-invalid' : '' }}" name="skilled_id" id="skilled_id" required>
                    @foreach($skilleds as $id => $entry)
                        <option value="{{ $id }}" {{ old('skilled_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('father_husband') ? 'is-invalid' : '' }}" type="text" name="father_husband" id="father_husband" value="{{ old('father_husband', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('photo') ? 'is-invalid' : '' }}" type="text" name="photo" id="photo" value="{{ old('photo', '') }}" required>
                @if($errors->has('photo'))
                    <div class="invalid-feedback">
                        {{ $errors->first('photo') }}
                    </div>
