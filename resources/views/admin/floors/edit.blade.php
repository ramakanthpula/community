    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.floors.update", [$floor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('block_name') ? 'is-invalid' : '' }}" name="block_name_id" id="block_name_id" required>
                    @foreach($block_names as $id => $block_name)
                        <option value="{{ $id }}" {{ (old('block_name_id') ? old('block_name_id') : $floor->block_name->id ?? '') == $id ? 'selected' : '' }}>{{ $block_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_of_units') ? 'is-invalid' : '' }}" type="text" name="no_of_units" id="no_of_units" value="{{ old('no_of_units', $floor->no_of_units) }}" required>
