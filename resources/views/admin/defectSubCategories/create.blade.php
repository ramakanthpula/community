...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.defectSubCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.defect-sub-categories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('defect_category') ? 'is-invalid' : '' }}" name="defect_category_id" id="defect_category_id">
                    @foreach($defect_categories as $id => $defect_category)
                        <option value="{{ $id }}" {{ old('defect_category_id') == $id ? 'selected' : '' }}>{{ $defect_category }}</option>
                    @endforeach
                </select>
