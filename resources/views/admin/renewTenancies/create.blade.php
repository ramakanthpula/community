...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.renewTenancy.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.renew-tenancies.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('floor_name') ? 'is-invalid' : '' }}" name="floor_name_id" id="floor_name_id" required>
                    @foreach($floor_names as $id => $floor_name)
                        <option value="{{ $id }}" {{ old('floor_name_id') == $id ? 'selected' : '' }}>{{ $floor_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('owner_unit') ? 'is-invalid' : '' }}" name="owner_unit_id" id="owner_unit_id" required>
                    @foreach($owner_units as $id => $owner_unit)
                        <option value="{{ $id }}" {{ old('owner_unit_id') == $id ? 'selected' : '' }}>{{ $owner_unit }}</option>
                    @endforeach
                </select>
