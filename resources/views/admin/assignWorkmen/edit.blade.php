...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.assignWorkman.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assign-workmen.update", [$assignWorkman->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                @foreach(App\Models\AssignWorkman::UNIT_ASSIGNMENT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('unit_assignment') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="unit_assignment_{{ $key }}" name="unit_assignment" value="{{ $key }}" {{ old('unit_assignment', $assignWorkman->unit_assignment) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('working_area') ? 'is-invalid' : '' }}" name="working_area" id="working_area">{{ old('working_area', $assignWorkman->working_area) }}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\AssignWorkman::WEEKLY_OFF_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('weekly_off') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="weekly_off_{{ $key }}" name="weekly_off" value="{{ $key }}" {{ old('weekly_off', $assignWorkman->weekly_off) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('contract_effective_date') ? 'is-invalid' : '' }}" type="text" name="contract_effective_date" id="contract_effective_date" value="{{ old('contract_effective_date', $assignWorkman->contract_effective_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('from_date') ? 'is-invalid' : '' }}" type="text" name="from_date" id="from_date" value="{{ old('from_date', $assignWorkman->from_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('to_date') ? 'is-invalid' : '' }}" type="text" name="to_date" id="to_date" value="{{ old('to_date', $assignWorkman->to_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gate_pass_status') ? 'is-invalid' : '' }}" name="gate_pass_status" id="gate_pass_status" required>
                    ...
                    @foreach(App\Models\AssignWorkman::GATE_PASS_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gate_pass_status', $assignWorkman->gate_pass_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
